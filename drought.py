...
indices_scenarios = np.floor(
    (
        vector_indices
        - indices_grid * (len(models) * len(scenarios))
...
def thornthwaite(lat, T, time):
    ...
    if np.any(I == 0):
        raise ValueError(
            "Heat index (I) is zero for some grid points. Cannot compute PET."
        )
    ...
    for date in time.astype("datetime64[D]"):
        ...
        daylight_hours.append(
            12 + 4 * np.sin((2 * np.pi * (month - 1) / 12) + (lat * np.pi / 180))
        )
    daylight_hours = np.array(daylight_hours)  # Convert to NumPy array

    # Expand daylight_hours to match the grid shape
    daylight_hours_grid = np.expand_dims(
        daylight_hours, axis=(1, 2)
    )  # Shape: (months, 1, 1)
    daylight_hours_grid = np.tile(
        daylight_hours_grid, (1, T.shape[1], T.shape[2])
    )  # Match grid shape

    # Compute PET for each grid point
    PET = (
        16
        * (10 * T / I[np.newaxis, :, :]) ** a[np.newaxis, :, :]
        * daylight_hours_grid
        / 12
    )
    ...
    for model in models:
        for scenario in scenarios:
            ...
            for tf_idx, tf in enumerate(time_frames):
                ...
                print(
                    f"Reading year {tf} for Model {model}, Scenario {scenario}",
                    flush=True,
                )
                ...
                if tasmax.size == 0 or np.isnan(tasmax).all():
                    ...
                    final_result = np.full(
                        (len(TR), len(lat_coords), len(lon_coords)), np.nan
                    )
                else:
                ...
                result_xr3.to_netcdf(
                    os.path.join(
                        output_folder,
                        f"spei3_quantiles_{model}_{scenario}_{tf}_n_{job_num}.nc",
                    )
                )
                result_xr12.to_netcdf(
                    os.path.join(
                        output_folder,
                        f"spei12_quantiles_{model}_{scenario}_{tf}_n_{job_num}.nc",
                    )
                )
